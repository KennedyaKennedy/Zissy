...
Welcome to the Zissy AI Project Drive. This project is designed for companionship, personalization, and high-performance desktop presence.
...
```mermaid
graph TD
    UserInput["User Voice/Text"] --> Detect["AI Engine Detection (Ollama/LM Studio)"]
    Detect --> Context["Memory Injection (JSON Profiles)"]
    Context --> LLM["Local Brain Generation"]
    LLM --> Parallel
    subgraph Parallel ["Parallel Multimodal Processing"]
        Voice["Voice Synthesis (MeloTTS)"]
        Face["Expression Loop (Blendshapes)"]
    end
    Voice --> Result["Visual/Audio Presence"]
    Face --> Result
    LLM --> Learning["Fact Extraction Loop"]
    Learning --> Memory[("Persistent Knowledge")]
```
